...
const WEBSOCKET_URL =
  process.env.NODE_ENV !== 'production'
    ? 'wss://localhost:7206/hub/votingResults'
...
const ResultContent = () => {
  ...
  return (
    <div className="flex flex-col items-center m-10">
      ...
      {votingData
        ? votingData?.votes.map((item, index) => {
            const firstItemWidth =
              (100 * item.firstVote.count) /
              (item.firstVote.count + item.secondVote.count)
            const secondItemWidth =
              (100 * item.secondVote.count) /
              (item.firstVote.count + item.secondVote.count)
